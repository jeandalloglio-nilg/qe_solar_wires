...
int main(int argc, char** argv) {
    ...
    const char* out_arg  = (argc >= 6) ? argv[5] : "overwrite"; // "overwrite" ou caminho de saída
    ...
    // (Opcional) Se seu enum tiver ColorDistributionMode, você pode tentar ativar um modo “linear”.
    // Nem todas as builds exigem isso; se sua enum não tiver estes símbolos, comente as linhas.
    // ACS_ThermalImage_setColorDistributionMode(img, ACS_ColorDistributionMode_TemperatureLinear);
    // ou
    // ACS_ThermalImage_setColorDistributionMode(img, ACS_ColorDistributionMode_SignalLinear);

    // Pegar Scale e setar faixa manual [tmin_c, tmax_c]
    ...
    // 0 = manter formato original. Se sua enum tiver RJPEG, você pode preferir isso:
